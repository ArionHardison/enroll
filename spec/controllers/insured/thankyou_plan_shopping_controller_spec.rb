...
RSpec.describe Insured::PlanShoppingsController, :type => :controller, dbclean: :after_each do
  ...
  describe 'GET thankyou', :dbclean => :around_each do
    ...
    before do
      ...
      allow_any_instance_of(UnassistedPlanCostDecorator).to receive(:total_ehb_premium).and_return(total_ehb_premium)
      sign_in(user)
    ...
    context '@pct value' do
      ...
      context "when elected aptc is 50% of total ehb premium if total ehb premium is greater than elected aptc" do
        it_behaves_like "sets @pct correctly", 196.88, 100
      end
    ...
    context '@max_credit value' do

      shared_examples "sets @max_credit correctly" do |elected_aptc, total_ehb_premium|
        let(:elected_aptc) { elected_aptc }
        let(:total_ehb_premium) { total_ehb_premium }
        let(:value) { [elected_aptc, total_ehb_premium].min }

        it "should set @max_aptc correctly" do
          expect(response).to have_http_status(:success)
          expect(assigns(:max_credit)).to eq(value)
        end
      end

      context "when elected aptc is greater than ehb premium" do
        it_behaves_like "sets @max_credit correctly", 150.0, 50
      end

      context "when elected aptc is less than ehb premium" do
        it_behaves_like "sets @max_credit correctly", 150.0, 500
      end
    end
  end
end