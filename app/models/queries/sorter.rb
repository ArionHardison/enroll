  ...
  module Sorter
    ...
    def sort_query(query, order_by)
      if aggregatable_columns&.key?(@order_by.keys.first)
        aggregate_sort(query, order_by) # perform the sort via aggregation framework and return the sorted array of Klass objects
      else
    ...
    def query_pipeline(query)
      return [] unless query.is_a?(Mongoid::Criteria)
    ...
    def aggregate_sort(query, order_by)
      pipeline = pipeline_for_sort_column(order_by).unshift(query_pipeline(query))
      # aggregate returns json, so we need to transform back to Klass objects for the mongoid datatable to handle
      sorted_ids = klass.collection.aggregate(pipeline).map { |doc| doc['_id'] }
